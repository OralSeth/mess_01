...
$exclude = @("Administrator","Public","Sabre Realty")
$exclude = $exclude -join '.*|.*'
$users = Get-ChildItem -Path C:\Users | Where-Object {($_.PSIsContainer) -And ($_.Name -notmatch $exclude)}
...
ForEach ($user in $users) {
    ...
    ForEach ($user in $users) {
        $un = $user.BaseName
        $folders = @()
        $folderNames = @("Desktop","Document","Picture","Music","Video","Favorite")
        ForEach ($name in $folderNames) {
            $folders += Get-ChildItem -Path "$($user.Fullname)" | Where-Object {($_.PSIsContainer) -And ($_.Name -like '*$name*')}
        }
        ForEach ($folder in $folders) {
                    
            $size = (Get-STFolderSize -Path $folder.FullName -Precision 2 | Select TotalGBytes).TotalGBytes

            Write-Output "$($env:COMPUTERNAME),$($un),$($folder),$($size)"
        }
    }
}
